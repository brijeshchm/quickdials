 ...
 public function index(Request $request, $city, $search_kw)
{
    try {
        ...
        $keyword = DB::table('keyword as k')
            ...
            ->select(
                ...
                'k.heading',
                'k.courseabout',
                'k.paragraph1', 'k.paragraph2',
                'k.paragraph3', 'k.paragraph4',
                'k.paragraph5', 'k.paragraph6',
                'p.parent_category',
                'c.child_category',
                'c.child_slug',
                'c.child_banner',
        ...
        if (!empty($keyword)) {
            ...
            $zones = DB::table('citylists')
                ...
                ->select('zones.id', 'zones.zone', 'zones.pincode')
                ->distinct()
	...
	public function index_old(Request $request, $city, $search_kw)
	{
		$city = ucwords(str_replace("-", " ", $city));
		...
		if (!empty($keyword)) {
			...
			return view('client.searchlist', ['clientsList' => $clientsList, 'subcategory' => $subcategory, 'reviewsClientsList' => $reviewsClientsList, 'searchedKW' => $search_kw, 'area' => $area, 'onlyClients' => $onlyClients, 'keyword' => $keyword, 'city' => $city, 'citiesList' => $cities, 'zones' => $zones, 'kwdsList' => $kwdsList]);
		} else {
