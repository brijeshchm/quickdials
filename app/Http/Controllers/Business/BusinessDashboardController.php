...
public function dashboard()
{
    ...
    */
    ...
    //     ->selectRaw('COUNT(*) as comment_count, COALESCE(SUM(rating),0) as total_rating')
    //     ->first();

    // $avgRating = 0;
    // $ratingCount = 0;

    // if ($rating && $rating->comment_count > 0) {
    //     $avgRating = round($rating->total_rating / $rating->comment_count, 1);
 ...
 $leads = DB::table('leads')
    ...
    $leads->getCollection()->transform(function ($lead) use (
        ...
        $lead->share_address =
            ...
            "Regarding your enquiry for {$keyword}.\n" .
            (!empty($address) ? "Visit us at: {$address}\n" : '') .
            (!empty($map) ? "Directions: {$map}" : '');

        // 🔹 Share Review
        $lead->share_review =
            "Greetings from {$businessName}.\n" .
            "Rated {$avgRating} ⭐ from {$ratingCount} reviews.\n" .
            "See more: {$profileUrl}";
